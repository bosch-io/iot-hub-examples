...
public class CreateTopic {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException, TimeoutException {
        ...
        iotHubClient.createTopic(solutionId, acl).thenAccept(topic -> {
            LOGGER.info("Create topic with path : <{}>", topic.getPath());
        }).get(10, TimeUnit.SECONDS);

        iotHubClient.createTopic(solutionId+"/"+entryTopic, acl).thenAccept(topic -> {
            LOGGER.info("Create topic with path : <{}>", topic.getPath());
        }).get(10, TimeUnit.SECONDS);
